    ...
    */
    struct warlock_botAI : public bot_ai
    {
        ...
        void DoNormalAttack(uint32 diff)
        {
            ...
            switch(me->GetRace()) {
            case RACE_BLOODELF:
                if (IsSpellReady(87269, diff)) {
                    doCast(opponent, 87269);
                    return;
                }
            case RACE_UNDEAD_PLAYER:
                if (IsSpellReady(87279, diff)) {
                    doCast(opponent, 87279);
                    return;
                }
            }
        ...
        void ApplyClassPassives() const override
        {
            ...
            RefreshAura(87253, me->GetRace() == RACE_GNOME);
            RefreshAura(87250, me->GetRace() == RACE_ORC);
            RefreshAura(87265, me->GetRace() == RACE_HUMAN);
