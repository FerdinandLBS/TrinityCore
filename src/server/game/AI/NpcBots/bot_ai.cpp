...
void bot_ai::ResetBotAI(uint8 resetType)
{
    ...
    if (!me->IsInWorld())
    {
        ...
        Events.AddEvent(teleHomeEvent, Events.CalculateTime(0s)); //make sure event will be deleted
...
void bot_ai::RezGroup(uint32 REZZ)
{
    ...
    for (GroupReference const* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
    {
        ...
        if (doCast(target, REZZ))//rezzing it
        {
            BotWhisper("正在复活你", player);
            if (player != master)
            {
                std::string rezstr = "正在复活 ";
...
void bot_ai::_listAuras(Player const* player, Unit const* unit) const
{
    ...
    botstring << "光环列表：" << unit->GetName().c_str() << "(职业: " << uint32(bot_pet_player_class) << "), ";
    ...
    else if (unit->GetTypeId() == TYPEID_UNIT && unit->ToCreature()->IsNPCBot())
    {
        ...
        botstring << (owner != unit ? owner->GetName() : "空");
    }
    ...
    for (uint8 i = STAT_STRENGTH; i != MAX_STATS; ++i)
    {
        ...
        botstring << "\n总计" << mystat << ": " << float(totalstat);
    }
    botstring.precision(2);
    botstring << "\n近战攻强: " << int32(unit->GetTotalAttackPowerValue(BASE_ATTACK));
    botstring << "\n远程攻强: " << int32(unit->GetTotalAttackPowerValue(RANGED_ATTACK));
    botstring << "\n护    甲: " << uint32(unit->GetArmor());
    botstring << "\n暴    击: " << float(unit->GetUnitCriticalChanceDone(BASE_ATTACK));
    botstring << "\n防    御: " << uint32(unit->GetDefenseSkillValue());
    botstring << "\n未命中率: " << float(unit->GetUnitMissChance());
    botstring << "\n闪    躲: " << float(unit->GetUnitDodgeChance(BASE_ATTACK, me));
    botstring << "\n招    架: " << float(unit->GetUnitParryChance(BASE_ATTACK, me));
    botstring << "\n格    挡: " << float(unit->GetUnitBlockChance(BASE_ATTACK, me));
    botstring << "\n格挡数值: " << uint32(unit->GetShieldBlockValue());
    botstring << "\n物理承伤: " << float(dmg_taken_phy * unit->GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN, SPELL_SCHOOL_MASK_NORMAL));
    botstring << "\n魔法承伤: " << float(dmg_taken_mag * unit->GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN, SPELL_SCHOOL_MASK_MAGIC));

    WeaponAttackType type = BASE_ATTACK;
    float attSpeed = (unit->GetAttackTime(type) * unit->m_modAttackSpeedPct[type]) / 1000.f;
    botstring << "\n主手武器: " << int32(unit->GetFloatValue(UNIT_FIELD_MINDAMAGE)) << " - " << int32(unit->GetFloatValue(UNIT_FIELD_MAXDAMAGE));
    botstring << "(" << float(unit->GetPctModifierValue(UNIT_MOD_DAMAGE_MAINHAND, BASE_PCT) * unit->GetPctModifierValue(UNIT_MOD_DAMAGE_MAINHAND, TOTAL_PCT)) << "%)";
    botstring << "\n速    度: " << float(attSpeed)
        << " (" << float(((unit->GetFloatValue(UNIT_FIELD_MINDAMAGE) + unit->GetFloatValue(UNIT_FIELD_MAXDAMAGE)) / 2) / attSpeed) << " 秒伤)";
    if (unit->haveOffhandWeapon())
    {
        type = OFF_ATTACK;
        attSpeed = (unit->GetAttackTime(type) * unit->m_modAttackSpeedPct[type]) / 1000.f;
        botstring << "\n副手武器: " << int32(unit->GetFloatValue(UNIT_FIELD_MINOFFHANDDAMAGE)) << " - " << int32(unit->GetFloatValue(UNIT_FIELD_MAXOFFHANDDAMAGE));
        botstring << "(" << float(unit->GetPctModifierValue(UNIT_MOD_DAMAGE_OFFHAND, BASE_PCT) * unit->GetPctModifierValue(UNIT_MOD_DAMAGE_OFFHAND, TOTAL_PCT)) << "%)";
        botstring << "\n速    度: " << float(attSpeed)
            << " (" << float(((unit->GetFloatValue(UNIT_FIELD_MINOFFHANDDAMAGE) + unit->GetFloatValue(UNIT_FIELD_MAXOFFHANDDAMAGE)) / 2) / attSpeed) << " 秒伤)";
    }
    if (unit != me ||
            ...
            _botclass != BOT_CLASS_SHAMAN))
    {
        type = RANGED_ATTACK;
        attSpeed = (unit->GetAttackTime(type) * unit->m_modAttackSpeedPct[type]) / 1000.f;
        botstring << "\n远程武器: " << int32(unit->GetFloatValue(UNIT_FIELD_MINRANGEDDAMAGE)) << " - " << int32(unit->GetFloatValue(UNIT_FIELD_MAXRANGEDDAMAGE));
        botstring << "(" << float(unit->GetPctModifierValue(UNIT_MOD_DAMAGE_RANGED, BASE_PCT) * unit->GetPctModifierValue(UNIT_MOD_DAMAGE_RANGED, TOTAL_PCT)) << "%)";
        botstring << "\n速度: " << float(attSpeed)
            << " (" << float(((unit->GetFloatValue(UNIT_FIELD_MINRANGEDDAMAGE) + unit->GetFloatValue(UNIT_FIELD_MAXRANGEDDAMAGE)) / 2) / attSpeed)
            << " (" << float(unit->GetTypeId() == TYPEID_PLAYER ? unit->ToPlayer()->GetAmmoDPS() : unit->ToCreature()->GetCreatureAmmoDPS()) << " 子弹) 秒伤)";
    }
    botstring << "\n基础生命: " << int32(unit->GetCreateHealth());
    botstring << "\n生命总量: " << int32(unit->GetMaxHealth());
    botstring << "\n基础魔法: " << int32(unit->GetCreateMana());
    botstring << "\n魔法总量: " << int32(unit->GetMaxPower(POWER_MANA));
    if (unit->GetMaxPower(POWER_MANA) > 1 && unit->GetPowerType() != POWER_MANA)
        botstring << "\n当前魔发: " << int32(unit->GetPower(POWER_MANA));

    if (unit == me)
    {
        botstring << "\n法术强度: " << int32(me->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC));
        botstring << "\n5秒生命恢复: " << int32(_getTotalBotStat(BOT_STAT_MOD_HEALTH_REGEN));
        if (me->GetMaxPower(POWER_MANA) > 1)
        {
            botstring << "\n施法时5秒法力恢复: " << float((_botclass == BOT_CLASS_SPHYNX ? -1.f : 1.f) * me->GetFloatValue(UNIT_FIELD_POWER_REGEN_INTERRUPTED_FLAT_MODIFIER) * sWorld->getRate(RATE_POWER_MANA) * 5.0f);
            botstring << "\n非施法5秒法力恢复: " << float((_botclass == BOT_CLASS_SPHYNX ? -1.f : 1.f) * me->GetFloatValue(UNIT_FIELD_POWER_REGEN_FLAT_MODIFIER) * sWorld->getRate(RATE_POWER_MANA) * 5.0f);
        }
        botstring << "\n急    速: " << (haste >= 0 ? "+" : "-") << float(haste) << "%";
        botstring << "\n命    中: +" << float(hit) << "%";
        botstring << "\n精    准: " << int32(expertise) << " (-" << float(float(expertise) * 0.25f) << "%)";
        botstring << "\n护甲穿透: " << float(me->GetCreatureArmorPenetrationCoef()) << "%";
        ...
        botstring << "\n主要职能: " << uint32(_roleMask & BOT_ROLE_MASK_MAIN);
        botstring << "\n收集职能: " << uint32(_roleMask & BOT_ROLE_MASK_GATHERING);

        botstring << "\nPvP击杀: " << uint32(_pvpKillsCount) << ", 玩家击杀: " << uint32(_playerKillsCount) << ", 总计: " << uint32(_killsCount);
...
void bot_ai::CalculateAttackPos(Unit const* target, Position& pos) const
{
    ...
    for (uint8 i = 0; i < 5; ++i)
    {
        target->GetNearPoint(me, x, y, z, dist, Position::NormalizeOrientation(angle), dist / 2);
...
void bot_ai::OnSpellHit(Unit* caster, SpellInfo const* spell)
{
    ...
    for (uint8 i = 0; i != MAX_SPELL_EFFECTS; ++i)
    {
        ...
        if (!spell->IsPositive() && caster->IsHostileTo(me) && me->IsHostileTo(caster)) {
            if (spell->Effects[i].Effect == SPELL_EFFECT_PERSISTENT_AREA_AURA && auraname == SPELL_AURA_PERIODIC_DAMAGE
...
bool bot_ai::OnGossipHello(Player* player, uint32 /*option*/)
{
    ...
    if (player->GetGUID().GetCounter() != _ownerGuid)
    {
        if (IAmFree())
        {
            ...
            if (_botclass == BOT_CLASS_SPHYNX)
            {
                message1 << "要冒险尝试吸引" << me->GetName() << "的注意力吗?";
                message2 << "<投币>";
            }
            else if (_botclass == BOT_CLASS_DREADLORD)
            {
                message1 << "要尝试诱惑" << me->GetName() << '吗?';
                message2 << "<试试>";
            }
            else
            {
                message1 << "要雇佣" << me->GetName() << "吗?";
                message2 << "<雇佣>";
            }
    ...
    if (_ownerGuid)
    {
        ...
        if (player == master)
        {
            ...
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "管理装备...", GOSSIP_SENDER_EQUIPMENT, GOSSIP_ACTION_INFO_DEF + 1);
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "职责管理...", GOSSIP_SENDER_ROLES_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "队形管理...", GOSSIP_SENDER_FORMATION, GOSSIP_ACTION_INFO_DEF + 1);
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "法术管理...", GOSSIP_SENDER_ABILITIES, GOSSIP_ACTION_INFO_DEF + 1);
            if (_botclass < BOT_CLASS_EX_START)
            {
                AddGossipItemFor(player, GOSSIP_ICON_TALK, "天赋管理...", GOSSIP_SENDER_SPEC, GOSSIP_ACTION_INFO_DEF + 1);
                AddGossipItemFor(player, GOSSIP_ICON_TALK, "给与消耗品...", GOSSIP_SENDER_USEITEM, GOSSIP_ACTION_INFO_DEF + 1);
            }
            ...
            if (!gr)
            {
                ...
                if (player->GetNpcBotsCount() > 1)
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<创建队伍 (所有雇佣单位)>", GOSSIP_SENDER_JOIN_GROUP, GOSSIP_ACTION_INFO_DEF + 2);
            }
            else if (!gr->IsMember(me->GetGUID()))
            {
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<添加到队伍>", GOSSIP_SENDER_JOIN_GROUP, GOSSIP_ACTION_INFO_DEF + 1);
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<添加所有单位到队伍>", GOSSIP_SENDER_JOIN_GROUP, GOSSIP_ACTION_INFO_DEF + 2);
            }
            else
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<踢出队伍>", GOSSIP_SENDER_LEAVE_GROUP, GOSSIP_ACTION_INFO_DEF + 1);

            //movement toggle
            if (HasBotCommandState(BOT_COMMAND_MASK_UNMOVING))
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "跟随我", GOSSIP_SENDER_FOLLOWME, GOSSIP_ACTION_INFO_DEF + 1);
            if (!HasBotCommandState(BOT_COMMAND_STAY))
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "保持原位", GOSSIP_SENDER_HOLDPOSITION, GOSSIP_ACTION_INFO_DEF + 1);
            if (!HasBotCommandState(BOT_COMMAND_FULLSTOP))
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "待在原地，什么也不做", GOSSIP_SENDER_DONOTHING, GOSSIP_ACTION_INFO_DEF + 1);
        }
        if (player == master || (gr && gr->IsMember(master->GetGUID())))
        {
            ...
            case BOT_CLASS_MAGE:
            {
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "我需要食物", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 1);
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "我需要水", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
                if (me->GetLevel() >= 70)
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "来个餐桌", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 3);
                menus = true;
                break;
            }
            case BOT_CLASS_ROGUE:
            ...
            case BOT_CLASS_WARLOCK:
            {
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "来个治疗石", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 1);
                if (me->GetLevel() >= 68)
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "来个灵魂之井", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 3);
                menus = true;
                break;
            }
            default:
        ...
        if (player == master)
        {
            ...
            case BOT_CLASS_ROGUE:
            {
                if (me->GetLevel() >= 20)
                {
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "你该刷新毒药了", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
                    AddGossipItemFor(player, GOSSIP_ICON_TALK, "<选择主手毒药>", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 3);
                    Item const* oweap = _equips[BOT_SLOT_OFFHAND];
                    if (oweap && oweap->GetTemplate()->Class == ITEM_CLASS_WEAPON)
                        AddGossipItemFor(player, GOSSIP_ICON_TALK, "<选择副手毒药>", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 4);
                }
                break;
            }
            case BOT_CLASS_SHAMAN:
            {
                if (me->GetLevel() >= 10)
                {
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "你该刷新武器强化了", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
                    AddGossipItemFor(player, GOSSIP_ICON_TALK, "<选择主手强化>", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 3);
                    Item const* oweap = _equips[BOT_SLOT_OFFHAND];
                    if (oweap && oweap->GetTemplate()->Class == ITEM_CLASS_WEAPON)
                        AddGossipItemFor(player, GOSSIP_ICON_TALK, "<选择副手强化>", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 4);
                }
                if (me->GetShapeshiftForm() != FORM_NONE)
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "取消狼魂形态", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 5);
                break;
            }
            case BOT_CLASS_DRUID:
            {
                if (me->GetShapeshiftForm() != FORM_NONE)
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "变回人形", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 1);
                break;
            }
            case BOT_CLASS_HUNTER:
            {
                if (me->GetLevel() >= 10)
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<选择宠物>", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
            ...
            astr << "确定要解雇" << me->GetName() << "? 你可能会后悔的...";
            player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, GOSSIP_ICON_TAXI, "你可以滚蛋了",
                GOSSIP_SENDER_DISMISS, GOSSIP_ACTION_INFO_DEF + 1, astr.str().c_str(), 0, false);

            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "跟紧我，该死的！", GOSSIP_SENDER_TROUBLESHOOTING, GOSSIP_ACTION_INFO_DEF + 1);
        }
    }
    ...
    {
        menus = true;
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<了解这个生物>", GOSSIP_SENDER_SCAN, GOSSIP_ACTION_INFO_DEF + 1);
    }
...
bool bot_ai::OnGossipSelect(Player* player, Creature* creature/* == me*/, uint32 sender, uint32 action)
{
    ...
    case GOSSIP_SENDER_CLASS:
    {
        ...
        case BOT_CLASS_MAGE:
        {
            ...
            if (option == 1 || option == 2) //food, water
            {
                ...
                if (!food)
                {
                    std::string errorstr = "我还不会制作";
                    errorstr += iswater ? "魔法水" : "魔法面包";
            ...
            else if (option == 3) //refreshment table
            {
                ...
                if (!tableSpellId)
                {
                    BotWhisper("技能禁用了", player);
                    break;
                }
                if (!IsSpellReady(43987, GetLastDiff(), false))
                {
                    BotWhisper("还在冷却呢", player);
                    break;
                }
        ...
        case BOT_CLASS_ROGUE:
        {
            ...
            if (action == 1)
            {
                ...
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "后退", 1, GOSSIP_ACTION_INFO_DEF + ++count);
            }
            else if (action == 2)
            {
            ...
            else if (action == 3)
            {
                ...
                for (uint32 i = BOTAI_MISC_ENCHANT_AVAILABLE_1; i <= BOTAI_MISC_ENCHANT_AVAILABLE_6; ++i)
                {
                    ...
                    if (uint32 possiblePoisonMaxRank = GetSpell(possiblePoison))
                    {
                        ...
                        while (k < 16 && availableInfo->SpellName[k][0] == 0) k++;
                        AddGossipItemFor(player, same ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, availableInfo->SpellName[k], GOSSIP_SENDER_CLASS_ACTION2, GOSSIP_ACTION_INFO_DEF + possiblePoison);
                    }
                }
                ...
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "后退", 1, GOSSIP_ACTION_INFO_DEF + 1);
            }
            else if (action == 4)
            {
                subMenu = true;
                ...
                for (uint32 i = BOTAI_MISC_ENCHANT_AVAILABLE_1; i <= BOTAI_MISC_ENCHANT_AVAILABLE_6; ++i)
                {
                    ...
                    if (uint32 possiblePoisonMaxRank = GetSpell(possiblePoison))
                    {
                        ...
                        while (k < 16 && availableInfo->SpellName[k][0] == 0) k++;
                        AddGossipItemFor(player, same ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, availableInfo->SpellName[k], GOSSIP_SENDER_CLASS_ACTION3, GOSSIP_ACTION_INFO_DEF + possiblePoison);
                    }
                }
            ...
            else if (action == 3)
            {
                ...
                for (uint32 i = BOTAI_MISC_ENCHANT_AVAILABLE_1; i <= BOTAI_MISC_ENCHANT_AVAILABLE_5; ++i)
                {
                    ...
                    if (uint32 possibleEcnhantMaxRank = GetSpell(possibleEnchant))
                    {
                        ...
                        while (k < 16 && availableInfo->SpellName[k][0] == 0) k++;
                        AddGossipItemFor(player, same ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, availableInfo->SpellName[k], GOSSIP_SENDER_CLASS_ACTION2, GOSSIP_ACTION_INFO_DEF + possibleEnchant);
                    }
                }
                AddGossipItemFor(player, isauto ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, "<自动>", GOSSIP_SENDER_CLASS_ACTION2, GOSSIP_ACTION_INFO_DEF + 0);
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "后退", 1, GOSSIP_ACTION_INFO_DEF + 1);
            ...
            else if (action == 4)
            {
                ...
                for (uint32 i = BOTAI_MISC_ENCHANT_AVAILABLE_1; i <= BOTAI_MISC_ENCHANT_AVAILABLE_5; ++i)
                {
                    ...
                    if (uint32 possibleEcnhantMaxRank = GetSpell(possibleEnchant))
                    {
                        ...
                        while (k < 16 && availableInfo->SpellName[k][0] == 0) k++;
                        AddGossipItemFor(player, same ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, availableInfo->SpellName[k], GOSSIP_SENDER_CLASS_ACTION3, GOSSIP_ACTION_INFO_DEF + possibleEnchant);
                    }
                }
                AddGossipItemFor(player, isauto ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, "<自动>", GOSSIP_SENDER_CLASS_ACTION3, GOSSIP_ACTION_INFO_DEF + 0);
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "后退", 1, GOSSIP_ACTION_INFO_DEF + 1);
        ...
        case BOT_CLASS_HUNTER:
        {
            ...
            if (action == 2)
            {
                ...
                AddGossipItemFor(player, noPet ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, "<无>", GOSSIP_SENDER_CLASS_ACTION4, GOSSIP_ACTION_INFO_DEF + BOT_PET_INVALID);
        ...
        case BOT_CLASS_WARLOCK:
        {
            ...
            else if (action == 3) //soulwell
            {
                ...
                if (!wellSpellId)
                {
                    BotWhisper("法术被禁用了", player);
                    break;
                }
                if (!IsSpellReady(29893, GetLastDiff(), false))
                {
                    BotWhisper("法术还没有准备好", player);
                    break;
                }
            ...
            if (!IsCasting())
            {
                ...
                if (result != SPELL_CAST_OK)
                {
                    ...
                    if (result == SPELL_FAILED_LOW_CASTLEVEL)
                        BotWhisper("不行，我水平还不够高...", player);
                    else
                        BotWhisper("呃，失败了？！", player);
                }
                else
                {
    ...
    case GOSSIP_SENDER_EQUIPMENT: //equips change s1: send what slots we can use
    {
        ...
        //general
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "给我看看你的装备", GOSSIP_SENDER_EQUIPMENT_LIST, GOSSIP_ACTION_INFO_DEF + 1);

        //auto-equip
        AddGossipItemFor(player, GOSSIP_ICON_TALK, "自动装备...", GOSSIP_SENDER_EQUIP_AUTOEQUIP, GOSSIP_ACTION_INFO_DEF + 1);

        //weapons
        AddGossipItemFor(player, GOSSIP_ICON_TALK, "主手...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_MAINHAND);
        if (_canUseOffHand())
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "副手...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_OFFHAND);
        if (_canUseRanged())
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "远程...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_RANGED);
        if (_canUseRelic())
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "圣像...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_RANGED);
        ...
        AddGossipItemFor(player, GOSSIP_ICON_TALK, "脚部...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_FEET);
        AddGossipItemFor(player, GOSSIP_ICON_TALK, "手腕...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_WRIST);
        AddGossipItemFor(player, GOSSIP_ICON_TALK, "手套...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_HANDS);

        if (IsHumanoidClass(_botclass))
        {
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "披风...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_BACK);
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "衬衣...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_BODY);
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "左手戒指...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_FINGER1);
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "右手戒指...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_FINGER2);
            ...
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "项链...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_NECK);
        }

        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "全部脱下", GOSSIP_SENDER_UNEQUIP_ALL, GOSSIP_ACTION_INFO_DEF + 1);
        if (creature->GetCreatureTemplate()->unit_flags2 & UNIT_FLAG2_MIRROR_IMAGE)
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "更新外观", GOSSIP_SENDER_MODEL_UPDATE, GOSSIP_ACTION_INFO_DEF + 1);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "后退", 1, GOSSIP_ACTION_INFO_DEF + 1);
    ...
    case GOSSIP_SENDER_EQUIPMENT_SHOW: //equips change s2: send list of equippable items
    {
        ...
        if (_equips[slot])
        {
            ...
            if (slot <= BOT_SLOT_RANGED)
            {
                if (einfo->ItemEntry[slot] != 0)
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "用原来的装备", GOSSIP_SENDER_EQUIP_RESET, action);
                else
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "卸下", GOSSIP_SENDER_UNEQUIP, action);
            }
        ...
        if (itemList.empty())
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "呃... 我也没什么好给你的", 0, GOSSIP_ACTION_INFO_DEF + 1);
        }
        else
        {
            uint32 counter = 0;
        ...
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "后退", GOSSIP_SENDER_EQUIPMENT, GOSSIP_ACTION_INFO_DEF + 2);
    ...
    case GOSSIP_SENDER_EQUIP_AUTOEQUIP:
    {
        ...
        if (itemList.empty())
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "呃... 我没什么好给你的", 0, GOSSIP_ACTION_INFO_DEF + 1);
        }
        else
        {
            uint32 counter = 0;
    ...
    case GOSSIP_SENDER_ROLES_GATHERING:
    {
        ...
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "后退", GOSSIP_SENDER_ROLES_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
    ...
    case GOSSIP_SENDER_ABILITIES:
    {
        ...
        if (HasAblitiesAllowedList())
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "管理技能许可...", GOSSIP_SENDER_ABILITIES_USAGE_LIST, GOSSIP_ACTION_INFO_DEF + 2);
        ...
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "更新", sender, action);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "后退", 1, GOSSIP_ACTION_INFO_DEF + 2);
    ...
    case GOSSIP_SENDER_ABILITIES_SPECIFICS_LIST:
    {
        ...
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "后退", GOSSIP_SENDER_ABILITIES, GOSSIP_ACTION_INFO_DEF + 1);
    ...
    case GOSSIP_SENDER_ABILITIES_USAGE_LIST:
    {
        ...
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "后退", GOSSIP_SENDER_ABILITIES, GOSSIP_ACTION_INFO_DEF + 2);
        break;
    }
    case GOSSIP_SENDER_SPEC_SET:
    {
        ...
        if (newSpec != _spec && newSpec >= BOT_SPEC_BEGIN && newSpec <= BOT_SPEC_END)
        {
            ...
            specMsg << "我正在切换" << BotTalentSpecStrings[_newspec - 1] << "天赋";
    ...
    case GOSSIP_SENDER_HIRE:
    {
        ...
        if (!reason)
        {
            if (_ownerGuid)
            {
                ...
                ChatHandler ch(player->GetSession());
                ch.PSendSysMessage("%s无法加入，除非雇主解雇%s", me->GetName().c_str(), (me->GetGender() == GENDER_MALE ? "他" : "她"));
                break;
            }

            if (_botclass == BOT_CLASS_DEATH_KNIGHT && player->GetLevel() < 55)
            {
                BotWhisper("滚开, 弱鸡", player);
                ChatHandler ch(player->GetSession());
                ch.PSendSysMessage("你需要达到55级才能雇佣%s", me->GetName().c_str());
                break;
            }
            if (_botclass == BOT_CLASS_SPHYNX && player->GetLevel() < 60)
            {
                std::string msg1 = me->GetName() + "没有被" + player->GetName() + "的行为打动";
                me->TextEmote(msg1.c_str());
                ChatHandler ch(player->GetSession());
                ch.PSendSysMessage("你需要达到60级才能雇佣%s", me->GetName().c_str());
                break;
            }
            if (_botclass == BOT_CLASS_ARCHMAGE && player->GetLevel() < 20)
            {
                BotWhisper("我可不会在无聊的事情上浪费时间", player);
                ChatHandler ch(player->GetSession());
                ch.PSendSysMessage("你需要达到20级才能雇佣%s", me->GetName().c_str());
                break;
            }
            ...
            if (_botclass == BOT_CLASS_DARK_RANGER && player->GetLevel() < 40)
            {
                //BotWhisper("placeholder", player);
                ChatHandler ch(player->GetSession());
                ch.PSendSysMessage("你需要达到40级才能雇佣%s", me->GetName().c_str());
                break;
            }
            ...
            if (SetBotOwner(player))
            {
                if (_botclass == BOT_CLASS_SPHYNX)
                {
                    std::string msg1 = me->GetName() + "发出刺耳的声音，开始跟随" + player->GetName();
                    me->TextEmote(msg1.c_str());
                }
                else
        ...
        else
        {
            ...
            case 2: //max npcbots exceed
                ch.PSendSysMessage("你无法雇佣更多的人了(%u)", BotMgr::GetMaxNpcBots());
                BotSay("...", player);
                break;
            ...
            case 4: //class bots exceed
            {
                ...
                ch.PSendSysMessage("你不能雇佣更多这个职业的人了! %u / %u", count, BotMgr::GetMaxClassBots());
                BotSay("...", player);
                break;
            }
            default:
    ...
    case GOSSIP_SENDER_DISMISS:
    {
        ...
        for (uint8 i = BOT_SLOT_MAINHAND; i != BOT_INVENTORY_SIZE; ++i)
        {
            ...
            {
                std::ostringstream estr;
                estr << "无法重置装备" << uint32(i) << " (" << _getNameForSlot(i) << ")! 无法遣散雇佣兵!";
                ChatHandler ch(player->GetSession());
    ...
    case GOSSIP_SENDER_FORMATION:
    {
        ...
        if (HasRole(BOT_ROLE_RANGED))
            AddGossipItemFor(player, GOSSIP_ICON_TALK, "攻击范围...", GOSSIP_SENDER_FORMATION_ATTACK, GOSSIP_ACTION_INFO_DEF + 2);

        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "后退", 1, GOSSIP_ACTION_INFO_DEF + 3);
        break;
    }
    ...
    case GOSSIP_SENDER_FORMATION_ATTACK:
    {
        ...
        AddGossipItemFor(player, mode == BOT_ATTACK_RANGE_SHORT ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, "近距离攻击", GOSSIP_SENDER_FORMATION_ATTACK_DISTANCE_SET, GOSSIP_ACTION_INFO_DEF + 1);
        AddGossipItemFor(player, mode == BOT_ATTACK_RANGE_LONG ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, "远距离攻击", GOSSIP_SENDER_FORMATION_ATTACK_DISTANCE_SET, GOSSIP_ACTION_INFO_DEF + 2);

        std::ostringstream diststr;
        if (mode == BOT_ATTACK_RANGE_EXACT)
            diststr << "指定 (当前: " << uint32(master->GetBotMgr()->GetBotExactAttackRange()) << ')';
        else
            diststr << "指定 (0-50)";
        ...
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "后退", 1, GOSSIP_ACTION_INFO_DEF + 4);
        break;
    }
    ...
    case GOSSIP_SENDER_TROUBLESHOOTING:
    {
        ...
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "移除buff...", GOSSIP_SENDER_TROUBLESHOOTING_FIX, GOSSIP_ACTION_INFO_DEF + 2);
    ...
    case GOSSIP_SENDER_DEBUG_ACTION:
    {
        ...
        case 4: //list roles
        {
            close = false;
            ChatHandler ch(player->GetSession());
            ch.PSendSysMessage("%s的职责:", me->GetName().c_str());
            for (uint16 i = BOT_MAX_ROLE; i != BOT_ROLE_NONE; i >>= 1)
            {
                if (_roleMask & i)
                {
        ...
        case 5: //list spells
        {
            ...
            for (BotSpellMap::const_iterator itr = myspells.begin(); itr != myspells.end(); ++itr)
            {
                ...
                sstr << " ID: " << itr->second->spellId << ", 基础: " << itr->first
                    << ", 冷却: " << itr->second->cooldown << ", 基础: " << std::max<uint32>(spellInfo->RecoveryTime, spellInfo->CategoryRecoveryTime);
        ...
        case 6: //reload config
        {
            ...
            ch.SendGlobalGMSysMessage("配置重新加载.");
            BotMgr::ReloadConfig();
            ch.SendGlobalGMSysMessage("雇佣兵配置重新加载.");
    ...
    case GOSSIP_SENDER_DEBUG:
    {
        ...
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<重置属性>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 2);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<属性列表>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 3);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<职责列表>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 4);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<法术列表>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 5);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<重载配置>", GOSSIP_SENDER_DEBUG_ACTION, GOSSIP_ACTION_INFO_DEF + 6);
...
void bot_ai::ApplyItemEnchantment(Item* item, EnchantmentSlot eslot, uint8 slot)
{
    ...
    {
        enchant_display_type = pEnchant->Effect[s]; // type
        enchant_amount = pEnchant->EffectPointsMin[s]; // amount
        enchant_spell_id = pEnchant->EffectArg[s];// spellid[s];
...
void bot_ai::_AddQuestLink(Player const* forPlayer, Quest const* quest, std::ostringstream& str) const
{
    ...
    str << "|cFFEFFD00|H任务:" << quest->GetQuestId() << ':' << quest->GetQuestLevel() << "|h[" << questTitle << "]|h|r";
}
//Unsused
void bot_ai::_AddWeaponSkillLink(Player const* forPlayer, SpellInfo const* spellInfo, std::ostringstream& str, uint32 skillid) const
{
    ...
    str << "|cff00ffff|H法术:" << spellInfo->Id << "|h[" << spellInfo->SpellName[loc] << " : " << master->GetSkillValue(skillid) << " /" << master->GetMaxSkillValue(skillid) << "]|h|r";
}
...
void bot_ai::_AddProfessionLink(Player const* forPlayer, SpellInfo const* spellInfo, std::ostringstream& str, uint32 skillId) const
{
    ...
    if (skillInfo)
    {
        ...
        str << "|cffffd000|H交易:" << spellInfo->Id << ':' << curValue << ':' << maxValue << ':' << master->GetGUID().GetCounter() << ":6AAAAAAAAAAAAAAAAAAAAAAOAADAAAAAAAAAAAAAAAAIAAAAAAAAA" << "|h[" << skillInfo->DisplayName[loc] << "]|h|r";
    }
}
//Localization
