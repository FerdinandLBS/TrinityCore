...
*/
    struct mage_botAI : public bot_ai
    {
        ...
        void DoNormalAttack(uint32 diff)
        {
            ...
            //Calm (no GCD)
            if (IsSpellReady(ARCANE_CALM, diff, false) && Rand() < 45 && !me->GetAura(ARCANE_CALM))
            {
                if (doCast(me, GetSpell(ARCANE_CALM)))
                    return;
            }
            ...
            //Arcane Missiles
            if (IsSpellReady(ARCANEMISSILES_1, diff) && dist < CalcSpellMaxRange(ARCANEMISSILES_1) && me->HasAura(44401))
            {
                int spellId = GetSpell(ARCANEMISSILES_1);
                const SpellInfo* si = sSpellMgr->GetSpellInfo(spellId);
                CastSpellExtraArgs args;
                args.AddSpellMod(SpellValueMod::SPELLVALUE_DURATION, 0 - si->DurationEntry->MaxDuration/2);

                for (int i = 0; i < 3; i++) {
                    if (si->Effects[i].Amplitude)
                        args.AddSpellMod(SpellValueMod::SPELLVALUE_EFFECT_PERIOD, 0 - si->Effects[i].Amplitude / 2);
                }
                if (SPELL_CAST_OK == me->CastSpell(opponent, GetSpell(ARCANEMISSILES_1), args)) {
                    me->RemoveAura(44401);
                    //me->CastSpell(opponent, GetSpell(ARCANEMISSILES_1), args);
            ...
            //Arcane Bargge
            if (IsSpellReady(ARCANE_BARRAGE, diff) && dist < CalcSpellMaxRange(ARCANEMISSILES_1) && Rand() < 80)
            {
                if (doCast(opponent, GetSpell(ARCANE_BARRAGE)))
                    return;
            }
            ...
            if (IsSpellReady(FROSTBOLT_1, diff) &&
                (_spec == BOT_SPEC_MAGE_FROST || (_spec == BOT_SPEC_MAGE_ARCANE && me->GetLevel() < 64)) &&
                dist < CalcSpellMaxRange(FROSTBOLT_1))
            {
                if (doCast(opponent, GetSpell(FROSTBOLT_1)))
                    return;
            }
            if (IsSpellReady(ARCANE_SHOCK_1, diff) && _spec == BOT_SPEC_MAGE_ARCANE && dist < CalcSpellMaxRange(ARCANE_SHOCK_1))
            {
                if (doCast(opponent, GetSpell(ARCANE_SHOCK_1)))
                    return;
            }
        ...
        void CheckTalents(uint32 diff) {
            ...
            switch (myRace) {
            case RACE_DRAENEI:
                if (!IsSpellReady(87216, diff))
                    return;

                doCast(me, 87216);
                break;
            case RACE_GNOME:
                if (!IsSpellReady(87208, diff) || !me->IsInCombat())
                    return;

                me->CastSpell(me, 87208);
                break;
            case RACE_UNDEAD_PLAYER:
                if (!IsSpellReady(87205, diff))
                    return;
        ...
        void ApplyClassSpellCastTimeMods(SpellInfo const* spellInfo, int32& casttime) const override
        {
            ...
            if (me->HasAura(ARCANE_CALM)) {
                timebonus += casttime;
                me->RemoveAura(ARCANE_CALM);
            }
 ...
 /*Special*/InitSpellMap(MANA_GEM_1);
  ...
  /*Talent*/lvl >= 60 && isFros ? InitSpellMap(DEEP_FREEZE_1) : RemoveSpell(DEEP_FREEZE_1);
            ...
            lvl >= 64 ? InitSpellMap(ARCANE_SHOCK_1) : RemoveSpell(ARCANE_SHOCK_1);

  /*Talent*/lvl >= 30 && isArca ? InitSpellMap(ARCANE_CALM) : RemoveSpell(ARCANE_CALM);
  /*Talent*///lvl >= 50 && isArca ? InitSpellMap(RAPPID_MISSLE) : RemoveSpell(RAPPID_MISSLE);
  /*Talent*/lvl >= 60 && isArca ? InitSpellMap(ARCANE_BARRAGE) : RemoveSpell(ARCANE_BARRAGE);
        ...
        void ApplyClassPassives() const override
        {
            ...
            RefreshAura(87210, me->GetRace() == RACE_GNOME);
            ...
            RefreshAura(RAPPID_MISSLE, isArca && level >= 50);
